...
# 3. CSS "FORA DA CAIXA" (Containers Nativos Estilizados + Tema Escuro Universal)
st.markdown("""
    <style>
    ...
    /* Transforma o container nativo no cartão escuro perfeito e garante que o botão fique dentro */
    ...
    /* Alinhamento exato na mesma linha horizontal das caixas de texto inferior */
    .sync-btn {
...
# 5. FRAGMENTO DOS TIMERS (Com botões nativamente dentro dos cartões)
...
def render_timer_grid():
    if len(st.session_state.lista_contas) > 0:
        ...
        for idx, conta in enumerate(st.session_state.lista_contas):
            ...
            with cols[idx % 5]:
                # Container nativo: garante que o botão "Iniciar" fique dentro da caixa do cartão
