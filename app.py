...
# 3. CSS GLOBAL E ROBUSTO (Elimina botões brancos e padroniza o design)
st.markdown("""
    <style>
    ...
    /* Estilização impecável dos containers nativos dos cartões no topo */
    ...
    /* Padronização universal de TODOS os botões do aplicativo no tema escuro */
...
# 5. FRAGMENTO DOS TIMERS (Com cards nativos estruturados)
        ...
        for idx, conta in enumerate(st.session_state.lista_contas):
            ...
            with cols[idx % 5]:
                with st.container(border=True):
                    st.markdown(f"<div style='font-size: 17px; font-weight: bold; color: #8b949e; text-align: center; margin-top: 5px;'>{nome_conta}</div>", unsafe_allow_html=True)
